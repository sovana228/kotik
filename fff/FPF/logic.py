...
class DatabaseManager:
    ...
    def get_winners_count(self, prize_id):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT COUNT(*) winners WHERE prize_id=?', (prize_id, ))
            return cur.fetchall()[0][0]
    ...
    def get_rating(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('''
    SELECT users.user_name, Count(winners.prize_id) AS cuont_prize FROM winners
    INNERS JOIN users on user.user_id=winners.user_id
